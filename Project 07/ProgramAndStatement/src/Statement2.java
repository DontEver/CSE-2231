...
public class Statement2 extends StatementSecondary {
    ...
    private void createNewRep() {
        ...
        this.rep = new Tree1<>();
        StatementLabel rootLabel = new StatementLabel(Kind.BLOCK);
        Sequence<Tree<StatementLabel>> children = this.rep.newSequenceOfTree();
        this.rep.assemble(rootLabel, children);
    ...
    public final Statement removeFromBlock(int pos) {
        ...
        Tree<StatementLabel> removedSubtree = this.rep.removeSubtree(pos);
        Statement2 removedStatement = new Statement2();
        removedStatement.rep = removedSubtree;
    ...
    public final void assembleIfElse(Condition c, Statement s1, Statement s2) {
        ...
        Statement2 thenStatement = (Statement2) s1;
        Statement2 elseStatement = (Statement2) s2;
        Sequence<Tree<StatementLabel>> children = this.rep.newSequenceOfTree();

        children.add(0, thenStatement.rep);
        children.add(1, elseStatement.rep);
        this.rep.assemble(new StatementLabel(Kind.IF_ELSE, c), children);

        thenStatement.createNewRep(); // clear input statement
        elseStatement.createNewRep();
    ...
    public final Condition disassembleIfElse(Statement s1, Statement s2) {
        ...
        StatementLabel label = this.rep.disassemble(children);
        thenStatement.rep = children.remove(0);
        elseStatement.rep = children.remove(0);
        this.createNewRep();
    ...
    public final void assembleWhile(Condition c, Statement s) {
        ...
        // casting s to Statement2 to work with representation
        Statement2 sAsStatement2 = (Statement2) s;

        // creating new sequence for children of WHILE statement
        Sequence<Tree<StatementLabel>> children = this.rep.newSequenceOfTree();

        // adding representation of s as only child of WHILE statement
        children.add(0, sAsStatement2.rep);

        // assembling WHILE statement with condition and single child
        this.rep.assemble(new StatementLabel(Kind.WHILE, c), children);

        // clearing original statement s to ensure solely part of WHILE structure
        sAsStatement2.createNewRep();
    }
    ...
    public final Condition disassembleWhile(Statement s) {
        ...
        // preparing to extract children (body) of WHILE statement
        Sequence<Tree<StatementLabel>> children = this.rep.newSequenceOfTree();

        // extracting label (contains condition) and body
        StatementLabel label = this.rep.disassemble(children);

        // casting s to Statement2 to modify to represent body of WHILE
        Statement2 bodyStatement = (Statement2) s;

        // assuming WHILE has one body statement
        bodyStatement.rep = children.remove(0);
        this.createNewRep();

        // returning condition part of WHILE statement
        return label.condition;
    }
    ...
    public final void assembleCall(String inst) {
        ...
        // create label for CALL with instruction name
        StatementLabel label = new StatementLabel(Kind.CALL, inst);

        // as CALL statements have no children, create empty sequence for children
        Sequence<Tree<StatementLabel>> children = this.rep.newSequenceOfTree();

        // assembling CALL statement with label and no children
        this.rep.assemble(label, children);
    ...
    public final String disassembleCall() {
        ...
        // preparing to extract potential children
        Sequence<Tree<StatementLabel>> children = this.rep.newSequenceOfTree();

        // extracting label which contains the instruction name
        StatementLabel label = this.rep.disassemble(children);

        // clearing representation to return only instruction name
        this.createNewRep();

        // returning instruction name part of CALL statement
